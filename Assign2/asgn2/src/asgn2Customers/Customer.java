...
public abstract class Customer {
	...
	public Customer(String name, String mobileNumber, int locationX, int locationY, String type) throws CustomerException{
		if(name.trim().isEmpty() || name.length() > 20){
			throw new CustomerException("Your name must contain characters, or your name length is too long!");
		}
		else if(mobileNumber.startsWith("0") == false || mobileNumber.length() == 10){
			throw new CustomerException("Your number must start with 0, or your mobile number must be 10 digits long!");
		}
		else{
